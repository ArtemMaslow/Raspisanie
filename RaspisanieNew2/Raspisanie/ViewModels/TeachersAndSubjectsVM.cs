...
namespace Raspisanie.ViewModels
{
    ...
    {
        private readonly INotifyingValue<List<TeachersAndSubjectsViewHelper<Subject>>> subjectList;
        private readonly INotifyingValue<int> subjectIndex;

        private readonly INotifyingValue<List<TeachersAndSubjectsViewHelper<DayOfWeek>>> dayList;
        ...
        {
            subjectList = this.Factory.Backing<List<TeachersAndSubjectsViewHelper<Subject>>>(nameof(SubjectList), null);
            subjectIndex = this.Factory.Backing(nameof(SubjectIndex), -1);

            dayList = this.Factory.Backing<List<TeachersAndSubjectsViewHelper<DayOfWeek>>>(nameof(DayList), null);
        ...
        public List<TeachersAndSubjectsViewHelper<Subject>> SubjectList { get { return subjectList.Value; } set { subjectList.Value = value; } }
        public int SubjectIndex { get { return subjectIndex.Value; } set { subjectIndex.Value = value; } }

        List<TeachersAndSubjectsViewHelper<DayOfWeek>> DayList { get { return dayList.Value; } set { dayList.Value = value; } }
