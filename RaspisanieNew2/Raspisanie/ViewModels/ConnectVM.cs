                            ...
                            if (con.State == System.Data.ConnectionState.Open)
                            {
                                using (FbTransaction dbtran = con.BeginTransaction())
                                {
                                    using (FbCommand createTables = new FbCommand())
                                    {
                                        ...
                                        " EXECUTE STATEMENT 'Create table TeachersAndSubjects(id_TAndS integer, id_teacher integer,subjectlist varchar(1000), daylist varchar(150),  primary key(id_TAndS), foreign key(id_teacher) references Teachers(id_teacher) ON DELETE CASCADE)';" +
