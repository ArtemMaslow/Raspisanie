...
///////////////////////////////////////  
        ...
        public void CreateDataBase()
        {
            ...
            {
                FbCommand createFacultyTable = new FbCommand("Create table Faculty (id_faculty integer, name_of_faculty char(35),  primary key(id_faculty))", con);
                FbCommand createDepartmentsTable = new FbCommand("Create table Departments(id_department integer,  name_of_department char(50),  id_faculty integer,  primary key(id_department),  foreign key(id_faculty) references faculty(id_faculty) ON DELETE CASCADE)", con);
                FbCommand createGroupsTable = new FbCommand("Create table Groups(id_group integer,    name_of_group char(50),    id_department integer,    primary key(id_group),    foreign key(id_department) references Departments(id_department) ON DELETE CASCADE)", con);
                FbCommand createSubjectsTable = new FbCommand("Create table Subjects(id_subject integer,    name_of_subject char(50),    id_department integer,    specific char(15),    primary key(id_subject),    foreign key(id_department) references Departments(id_department) ON DELETE CASCADE)",con);
                FbCommand createTeachersTable = new FbCommand("Create table Teachers(id_teacher integer,    fio char(50),    post char(25),    primary key(id_teacher))", con);
                FbCommand createClassroomsTable = new FbCommand("create table Classrooms(id_classroom integer,    number_of_classroom char(10),    id_department integer,    specific char(20),    primary key(id_classroom),    foreign key(id_department) references Departments(id_department) ON DELETE CASCADE)", con);
